...
初期実装段階。閲覧中のタブで利用するHLSストリームを検出し、PopupからmacOSのNative Hostを通じて保存できる。保存中、完了、失敗、キャンセルの状態と結果もPopupで確認できる。「保存」はダウンロード内の既定保存先とランダムID付きファイル名を使い、「設定して保存」では今回に限り拡張子を省略できるファイル名と、ムービーまたはダウンロードの保存先を選べる。
...
- 保存: ダウンロード内の既定保存先とランダムID付きファイル名を使用する
- 設定して保存: 今回の保存に限りファイル名と保存先を変更する

ファイル名と保存先の詳細は、macOS Native Host の開発用登録を参照する。
...
- URL単体をffmpegへ渡して取得できるストリーム（マスタープレイリストを含む）
...
- Firefox拡張から任意のffmpeg引数を渡さず、URL、ファイル名、保存先IDなど必要な項目だけを渡す
