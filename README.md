...
初期実装段階。閲覧中のタブで利用するHLSストリームを検出し、PopupからmacOSのNative Hostを通じて保存できる。保存中、完了、失敗、キャンセルの状態と結果もPopupで確認できる。「保存」はムービー内の既定保存先とランダムID付きファイル名を使い、「設定して保存」では今回に限り `.mp4` のファイル名と、ムービーまたはダウンロードの保存先を選べる。画質の設定は後続で扱う。
...
このスクリプトは `~/Library/Application Support/Mozilla/NativeMessagingHosts/` に、固定した拡張IDだけを許可する manifest を作成する。あわせて `~/Library/Application Support/Media Stream Bridge/` に、実行時の `PATH` に依存しないNode.jsとffmpegの絶対パスを固定したランチャーを生成する。通常の保存では Host は `~/Movies/Media Stream Bridge/` にランダムID付きの `.mp4` を保存する。設定して保存では単一の `.mp4` ファイル名を指定し、今回に限り `~/Movies/Media Stream Bridge/` または `~/Downloads/Media Stream Bridge/` を選べる。指定名が無効または同名のファイルが既に存在する場合は保存せず、既存ファイルを上書きしない。登録後は拡張機能を再読み込みする。
