...
閲覧中のページで検出したストリームを、Firefox 上からローカルへ保存するための拡張機能です。

## 機能

- 閲覧中のタブで検出した HLS ストリーム候補を表示する
- 候補が一つなら自動選択し、複数なら保存する候補を選べる
- ページタイトルと保存日時を使った既定名で、既定の保存先へすぐに保存できる
- ファイル名と保存先を指定して保存できる
...
`保存` は `~/Downloads/Media Stream Bridge/` に `<ページタイトル>_YYYYMMDDHHmmss.mp4` 形式で保存する。ページタイトルを利用できない場合は、ランダムID付きの名前を使う。`設定して保存` では同じ形式の名前が初期表示され、今回に限りファイル名と `Downloads` または `Movies` を選べる。拡張子を省略した名前には `.mp4` を付与する。
