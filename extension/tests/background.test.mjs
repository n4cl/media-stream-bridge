...
test("通信で検出したHLS候補をPopup向けメッセージへ返す", async () => {
  ...
  const configuredDestinationSave = receiveMessage({
    type: "save:start",
    tabId: 22,
  ...
  assert.deepEqual(await receiveMessage({ type: "save:status", tabId: 22 }), {
    ok: true,
    job: { state: "failed", error: "output-directory-unavailable", saveId: "save-22" },
  });

  const connectionsBeforeInvalidDestination = nativeConnectionCount;
  assert.deepEqual(
    await receiveMessage({
      type: "save:start",
      tabId: 23,
      hlsUrl: "https://example.com/invalid-destination.m3u8",
      destination: 1,
    }),
    { ok: false, error: "invalid-save-destination" },
  );
  assert.equal(nativeConnectionCount, connectionsBeforeInvalidDestination);
  assert.deepEqual(await receiveMessage({ type: "save:status", tabId: 23 }), {
    ok: true,
    job: { state: "failed", error: "invalid-save-destination" },
  });
