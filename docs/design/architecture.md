...
- 既定の設定で保存を開始するか、今回のファイル名を指定して保存する。
...
ストリームURLは認証情報を含む可能性があるため、完全な値を永続ログへ残さない。保存開始要求では、任意の `outputFileName` として `.mp4` の単一ファイル名だけを渡せる。Native Host はこの値を再検証し、保存先ディレクトリと最終パスを自ら決定する。ffmpegは同じディレクトリ内のジョブ固有の途中ファイルへ出力し、成功後に上書きしない排他的操作で最終名を公開する。同名競合、失敗、キャンセルでは既存ファイルを変更せず、今回の途中ファイルだけを削除する。
