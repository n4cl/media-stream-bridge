...
- Native Host から受け取った進捗と結果を Popup へ中継する。
...
- デフォルト設定による保存と、今回の設定を確認してからの保存を開始する。
- 保存の進捗、完了、失敗を表示する。
...
- 機械的に読み取れる形式で進捗を Native Host へ出力する。
...
Firefox WebExtension は、ストリームURL、論理的なファイル名、選択した保存設定など、操作に必要な値だけを渡す。Native Host は受け取った値を信頼せず検証し、実際のファイルパスとプロセス起動を決定する。
