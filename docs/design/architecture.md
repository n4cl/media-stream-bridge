...
- 既定の設定で保存を開始するか、今回のファイル名と保存先を指定して保存する。
...
ストリームURLは認証情報を含む可能性があるため、完全な値を永続ログへ残さない。保存開始要求では、任意の `outputFileName` として拡張子を省略できる単一ファイル名を渡せる。Native Host は `.mp4` がない入力へこれを付与し、最終名を検証する。保存先は任意パスではなく `movies` または `downloads` のIDだけを渡せ、未指定時は `downloads` を使う。Native Host は各値を再検証し、ホームディレクトリ基準の保存先と最終パスを自ら決定する。ffmpegは選択した保存先と同じディレクトリ内のジョブ固有の途中ファイルへ出力し、成功後に上書きしない排他的操作で最終名を公開する。同名競合、失敗、キャンセルでは既存ファイルを変更せず、今回の途中ファイルだけを削除する。
...
将来必要になる追加の保存設定は、その必要性が判明した時点で検討する。これらは最小の検証実装から得た事実を基に決める。
